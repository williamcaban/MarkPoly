...
class ContentTranslator:
    ...
    def _reconstruct_yaml(self, yaml_data: Dict[str, Any], translated_blocks: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Reconstruct YAML with translated values.
        
        Args:
            yaml_data: Original YAML data
            translated_blocks: List of translated blocks with path information
            
        Returns:
            Updated YAML data with translations
        """
        ...
        for block in translated_blocks:
            ...
            if isinstance(current, dict) and last_key in current:
                current[last_key] = block["content"]
            elif isinstance(current, list):
                idx = int(last_key)
                if idx < len(current):
                    current[idx] = block["content"]
    ...
    def translate_content(
        ...
        """
        Translate a file (markdown or YAML) and save the result.
        ...
        try:
            ...
            is_yaml = self._is_yaml_file(input_file)
            
            if is_yaml:
                if verbose:
                    logger.info(f"Processing {input_file} as YAML")
                ...
                if is_yaml:
                    try:
                        ...
                        original_yaml = yaml.safe_load(content)
                        blocks = self.split_yaml_into_blocks(content)
                        ...
                        for i, block in enumerate(blocks, 1):
                            if verbose:
                                block_preview = str(block["content"])[:50] + "..." if len(str(block["content"])) > 50 else str(block["content"])
                                logger.info(f"Processing YAML block {i}/{total_blocks}: {block_preview}")
                            
                            translated = self.translate_yaml_block(block)
                            translated_blocks.append(translated)
                        
                        # Reconstruct the YAML structure with translated values
                        translated_yaml = self._reconstruct_yaml(original_yaml, translated_blocks)
                        
                        # Convert back to YAML string with preserved formatting
                        translated_content = yaml.dump(translated_yaml, allow_unicode=True, sort_keys=False)
                    except Exception as e:
                        logger.error(f"YAML translation failed: {str(e)}")
                        raise
                else:
                    if verbose:
                        logger.info(f"Processing {input_file} as Markdown")
                    
                    # Split the markdown content into blocks
                    ...
                    for i, block in enumerate(blocks, 1):
                        if verbose:
                            block_preview = block["content"][:50] + "..." if len(block["content"]) > 50 else block["content"]
                            logger.info(f"Processing block {i}/{total_blocks} ({block['type']}): {block_preview}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Translate markdown and YAML files between languages using AI.',
    ...
    parser.add_argument(
        'input_file',
        type=str,
        help='Path to the input file (markdown or YAML)'
    )
    ...
    parser.add_argument(
        '-o', '--output',
        type=str,
        help='Path to save the translated file. If not provided, will use input_file_translated.md/yml'
    )
    parser.add_argument(
        '--source-lang',
        type=str,
